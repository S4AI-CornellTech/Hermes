...
def parse_arguments():
    ...
    parser.add_argument("--frequency", type=int, default=1000, help="Frequency for DVFS scaling")
...
def main():
    ...
    output_file = os.path.join(args.output_dir, "hermes_cluster_latency_dvfs.csv")
    ...
    write_header = not (os.path.isfile(output_file) and os.path.getsize(output_file) > 0)

    with open(output_file, mode='a', newline='') as file:
        fieldnames = ["Frequency", "Cluster ID", "Batch Size", "nprobe", "Retrieved Docs", "Num Threads", "Avg Retrieval Latency (s)"]
