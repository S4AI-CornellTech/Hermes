...
def plot_accuracy(file_path: str, output_dir: str, sample_nprobe_val: int, deep_nprobe_val: int) -> None:
    ...
    Load CSV data and create two subplots showing NDCG accuracy for both:
      - Sample nProbe sweep (rows with Deep nProbe == deep_nprobe_val)
    ...
    df_sample = df[df['Deep nProbe'] == deep_nprobe_val]
    ...
    axes[0].set_title(f"Sample nProbe Sweep\n(Deep nProbe = {deep_nprobe_val})", fontsize=12, fontweight='bold')
    axes[0].grid(True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot Latency analysis from CSV data"
    )
